...
const AdminChapterList = () => {
  ...
  const uploadChapter = async () => {
    ...
    try {
      const response = await fetch(`${config.backendUrl}/api/chapters`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setChapterList((prevList) => [...prevList, newWork]);
        setShowCreate(false);
        return newWork.id;
      }
      alert('请输入标题')
  ...
  const deleteData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setChapterList((prevList) => prevList.filter((item) => item.id !== deleteId));
      }
    } catch (error) {
  ...
  const handleSelect = (selectedOption) => {
    ...
    navigate(`/admin/chapter/${selectedOption.id}`)
  };

  const fetchChaList = async (id) => {
    try {
      const response = await fetch(`${config.backendUrl}/api/chapters/${id}`);
      const data = await response.json();
      setChapterList(data);
    } catch (error) {
      console.error('Failed to fetch data:', error);
  ...
  return (
    <div className='listPage-container'>
      <div className='header-container'>
        <button id='single1' className='small-button' onClick={handleAddWork}>新建章节</button>
        <Dropdown
          options={workList}
        ...
        <div className='editList-container'>
          {chapterList.map((item, index) => (
            <EditItem
              key={item.id}
              text={item.title}
              editClick={() => navigate(`/admin/chapter/${work_id}/${item.id}`)}
          ...
          <div className="create-work">
            <div className="createWork-content">
              ...
              <div className="createWork-buttons">
                <button className="small-button" onClick={createChapter}>确认</button>
